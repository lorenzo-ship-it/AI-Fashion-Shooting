...
import {
  buildBaseAnglePrompt,
  buildBaseMotherPrompt,
  buildDetailPrompt,
  buildLowerBodyPrompt,
  buildUpperBodyPrompt,
  buildVariantAnglePrompt,
  buildVariantDetailPrompt,
  buildVariantMotherPrompt,
} from './prompts';
  ...
  for (const block of outfit.blocks.filter((b) => b.type === BlockType.HANGING_VARIANT)) {
    ...
    for (const variant of relatedVariants) {
      ...
      for (const angle of angleList) {
        ...
        if (!mannequin) continue;
        const promptCategory = garmentLabel(garment.category) as PromptContext['category'];
        const prompt = `${buildVariantAnglePrompt({ angle, view, hexColor, category: promptCategory })} Focus on the ${garmentLabel(garment.category)} article ${garment.articleCode}.`;
        tasks.push({
          id: `${variant.id}-${angle}`,
          prompt,
