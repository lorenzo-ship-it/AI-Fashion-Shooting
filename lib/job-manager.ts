...
import type { PromptContent } from './gemini';
...
const resolveReference = async (jobId: string, runtime: JobRuntime, ref: ReferenceSource): Promise<PromptContent> => {
  if (ref.kind === 'upload') {
...
const executeTask = async (jobId: string, runtime: JobRuntime, task: GenerationTask) => {
  ...
  const contents: PromptContent[] = [{ text: SYSTEM_PROMPT }, { text: task.prompt }];
  ...
  while (attempt < 3) {
    try {
      ...
      const fileRelative = join('jobs', jobId, `${task.outputName}-${randomUUID()}.png`);
