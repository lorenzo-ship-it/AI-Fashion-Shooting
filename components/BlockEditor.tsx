...
export function BlockEditor({ blocks, onChange }: BlockEditorProps) {
  const toggleType = (blockId: string) => {
    const updated: EditableBlock[] = blocks.map((block) =>
      block.id === blockId
  ...
  const moveBlock = (blockId: string, direction: -1 | 1) => {
    ...
    const remapped: EditableBlock[] = newBlocks.map((b, idx) => ({ ...b, order: idx }));
    onChange(remapped);
  };
              ...
              <figure key={image.uploadId} className="rounded-lg bg-slate-800 border border-slate-700 p-2 text-xs text-slate-300">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    value={image.role ?? ''}
                    onChange={(event) => {
                      const selectedRole = (event.target.value || undefined) as
                        | 'front'
                        | 'side'
                        | 'back'
                        | 'detail'
                        | undefined;
                      const updated: EditableBlock[] = blocks.map((blockEntry) =>
